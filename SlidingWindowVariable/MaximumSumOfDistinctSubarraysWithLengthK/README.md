Below is clear documentation for your implementation.
...
1. Initialize:
   ...
   * `seen` → set to track elements in the window
   * `L` → left pointer of the window
   * `total` → running sum of the window
   * `max_sum` → result
...
class Solution:
    def maximumSubarraySum(self, nums: list[int], k: int) -> int:
        ...
        for R in range(len(nums)):
            ...
            # Shrink window if duplicate exists or window exceeds size k
...
**Space Complexity:**
`O(k)` — the set stores at most `k` elements.

---

### Why This Approach Is Clean

* Uses a **single sliding window pass**.
* Maintains a **strong window invariant** (always valid).
* Avoids frequency maps by **preventing duplicates immediately**.
* Keeps sum updates **O(1)** with incremental tracking.
