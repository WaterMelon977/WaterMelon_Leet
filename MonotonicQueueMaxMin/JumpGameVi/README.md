...
[LeetCode Problem Link](https://leetcode.com/problems/jump-game-vi/)
...
- Transition: from index `i`, we could have jumped from any `j` in `[i-k, i-1]`.
  So: `dp[i] = nums[i] + max(dp[j])` for `j â [i-k, i-1]`.
- Naively computing the max each time costs `O(k)` per step â `O(nk)`.
...
## Complexity
Time: **O(n)** â each index enters and leaves the deque at most once.
Space: **O(n)** â the `dp` array stores scores for each position.