...
You don’t need to simulate jumps. Track the **maximum reachable boundary** as you scan
the array.


**Why efficient**

The greedy approach keeps only the farthest reachable index and processes each element
once.
...
means the current index lies **beyond the reachable boundary**, so we cannot land there,
making the goal unreachable.

Edge-case handling: if the array has length `1`, we are already at the last index, so
